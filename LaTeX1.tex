...
\begin{equation}\label{equ:ecommunity}
 E_{c,j}=\frac{\Sigma_j\gamma_{i,c} OR_{i,j}}{\Sigma_j \gamma_{i,c} X_{i,j}}
\end{equation}
...
We have two important parameters in our models $\tau$ and $K$. In order to see the effects of these parameters, we set $\tau=0.2,0.,5,1.0,1.5,2.0$ and $K=5,10,15,20$. The $NDCG@5$ result of model CPC is shown in Fig.~\ref{fig:parameter}. We can conclude that (1) the performance is quite stable. No matter how we set the parameters,  $NDCG@5>0.74$, which is better than all non-ranking based recommender systems. This again demonstrate the superiority of our models.
(2) The best result is achieved when $\tau=1.5,K=10$. It suggests that the hardcore effect is relative strong as $\tau>1$. \begin{figure}[htbp]
\centering
\subfigure[$\tau$]{
